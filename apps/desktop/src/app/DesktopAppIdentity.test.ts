...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Stofloos.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Stofloos.app/Contents/Resources",
          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("Stofloos (Alpha)"),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Stofloos (Alpha)");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/Stofloos (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Stofloos (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Stofloos (Alpha)");
